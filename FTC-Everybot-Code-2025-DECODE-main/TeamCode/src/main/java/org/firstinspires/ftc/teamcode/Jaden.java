...
@Configurable
@TeleOp(name = "Jaden", group = "Teleop")
public class Jaden extends OpMode {
    ...
    public void init() {
        ...
        // Initialize automated path to move forward 10 units in X and Y
        pathChain = () -> {
            Pose currentPose = follower.getPose();
            Pose targetPose = new Pose(currentPose.getX() + 10, currentPose.getY() + 10, currentPose.getHeading());

            return follower.pathBuilder()
                    .addPath(new Path(new BezierLine(follower::getPose, targetPose)))
                    .setHeadingInterpolation(HeadingInterpolator.constant(follower.getHeading()))
                    .build();
        };
        ...
        telemetry.addData("Info", "Press DPAD_UP to move forward 10 units in X and Y");
        telemetry.update();
    }
    ...
    public void loop() {
        ...
        // Automated PathFollowing - Move forward 10 units in X and Y (triggered by dpad_up)
        if (gamepad1.dpad_up && !moveExecuted) {
        ...
        if (automatedDrive && (gamepad1.dpad_down || !follower.isBusy())) {
            ...
            if (gamepad1.dpad_down) {
                moveExecuted = false; // Reset so move can be executed again
            }
        }

        // Reset move flag to allow repeated moves
        if (!gamepad1.dpad_up && moveExecuted && !automatedDrive) {
            moveExecuted = false;
        }
        ...
        telemetry.addData("Move Command", moveExecuted ? "EXECUTING" : "READY");
        ...
        telemetry.addData("", "");
        telemetry.addData("Controls", "DPAD_UP: Move +10 X,Y | DPAD_DOWN: Cancel");
        telemetry.update();
